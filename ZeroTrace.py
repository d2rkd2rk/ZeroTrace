#!/usr/bin/env python3
"""
AI-Powered Linux NIDS with Real-Time Rich Dashboard & Active Firewall Defense
-----------------------------------------------------------------------------
...
try:
    ...
    print("[-] Missing dependencies! Install them using: pip install scapy rich joblib numpy")
    sys.exit(1)

if os.geteuid() != 0:
    print("[-] Error: This tool must be executed with root/sudo privileges!")
    sys.exit(1)
...
class NIDSDetector:
    ...
    def predict(self, features):
        if self.model is not None:
            ...
            proto, dst_port, flags, rate = features[0][1], features[0][3], features[0][4], features[0][5]
            if rate > 100 or flags == 2:
                return 1, "DoS / DDoS Attack"
            elif dst_port in [22, 21] and rate > 20:
                return 3, "Brute Force"
            elif flags == 0 or (flags == 2 and rate > 50):
...
class NIDSEngine:
    ...
    def build_dashboard(self) -> Layout:
        ...
        mode_str = "[bold red]Auto-Block ON[/bold red]" if self.auto_block else "[bold yellow]Manual-Block Mode[/bold yellow]"
        model_status = "[bold green]Loaded (.pkl)[/bold green]" if self.detector.model else "[bold yellow]Heuristic/Simulated[/bold yellow]"
        header_text = Text(f"AI-NIDS Linux Monitor | IFACE: {self.interface} | Mode: {mode_str} | AI Model: {model_status}", justify="center", style="bold white")
        ...
        stats_table.add_row("Total Packets", str(self.total_packets))
        ...
        blocked_list = "\n".join(list(self.firewall.blocked_ips)[-5:]) if self.firewall.blocked_ips else "No active blocks"
    ...
    def start(self):
        ...
        try:
            ...
            self.running = False
            self.console.print("\n[bold yellow][*] Shutting down NIDS Monitor... Clean exit.[/bold yellow]")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="AI Network Intrusion Detection System for Linux")
